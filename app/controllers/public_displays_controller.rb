...
class PublicDisplaysController < ApplicationController
  ...
  def show
    ...
    # A public kiosk token can never promote a featured asset beyond the
    # visibility of the post or group where it was originally shared.
    @media_assets = @site.media_assets.approved.where(featured: true).with_attached_file.order(created_at: :desc).select(&:publicly_accessible?).first(24)

...
Media originals are kept unchanged. Gallery, feed, and kiosk images use Vips-generated, metadata-stripped derivatives; only authorized staff can download an original. Members can request an asynchronous album ZIP containing web-sized images (and supplied video files); staff may instead request an original-file ZIP. ZIPs are tied to the requesting account, re-check every source asset's visibility before download, and expire after seven days. V1 intentionally serves accepted video uploads in their supplied browser-playable form rather than running an unmonitored ffmpeg/transcoding pipeline. Verify target browser compatibility with a short test upload before the event.
